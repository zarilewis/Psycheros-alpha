...
- **Problem**: `truncateSSEData()` in `convertToSSEEvent()` was called on already-serialized JSON (e.g., `JSON.stringify(toolResult)` then truncated the string). When a tool result exceeded 100KB (`MAX_SSE_MESSAGE_SIZE`), the serialized JSON was cut mid-structure, producing invalid JSON. On the client side, `JSON.parse(data)` threw inside a try-catch that only logged to `console.error` — so tool results silently vanished from the UI with no visible error. The LLM still processed the full result in its agentic loop context, so the server-side behavior was correct, but the user saw nothing. This was intermittent because it only triggered when tool results were large enough (web search results, identity file reads, image descriptions). Identity updates were especially vulnerable due to file content sizes.
- **Location**: `src/server/routes.ts` — `truncateSSEData()`, `convertToSSEEvent()`; `src/pulse/engine.ts` — Pulse tool_result broadcast
- **Fix**: For `tool_result` events, the `content` field is now truncated **before** JSON serialization (capped at 50KB with a `[... N characters truncated]` notice). The same pre-serialization truncation is applied to `tool_call` arguments. The resulting JSON is always valid and parseable by the client. The LLM still receives the full untruncated content in its context — truncation only affects what's sent to the browser via SSE. The Pulse engine's tool_result broadcast (via `EventBroadcaster`) received the same fix. `truncateSSEData()` is retained as a last-resort safety net for non-JSON event types.

### Mobile keyboard popup and input text disappearing during typing (High — UX)
- **Problem**: The `visibilitychange` handler (which fires on Android when the virtual keyboard opens/closes) called `loadConversationFromUrl()`, replacing the entire `#chat` innerHTML and destroying/recreating the textarea. The save/restore logic for unsent text caused a visible flash (text briefly disappearing then reappearing) and the keyboard would dismiss and sometimes re-trigger visibility events. Additionally, `initPersistentSSE()` was called unconditionally on every `visible` transition, tearing down healthy EventSource connections unnecessarily.
...
- **Fix**: When the `done` handler receives an event but `pulseAssistantEl` is null, it now checks the module-level `pulseStreamingPulseId`. If set, it clears it and calls `exitPulseStreamingMode()` to unstick the UI. Combined with the conditional SSE reconnect fix above (which avoids tearing down healthy connections), this both prevents the issue and recovers from it if it occurs.
