...
# Copy lock files generated by deno install in deps stage
# Psycheros deno.lock is gitignored so the source COPY above doesn't include it.
# Without these, deno run re-resolves every npm package from the registry at startup.
COPY --from=deps /app/Psycheros/deno.lock ./Psycheros/deno.lock
COPY --from=deps /app/entity-core/deno.lock ./entity-core/deno.lock
