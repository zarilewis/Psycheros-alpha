...
function parseSnapshotFilename(category: SnapshotCategory, filename: string): SnapshotMeta | null {
  ...
  return {
    ...
    source: "psycheros" as SnapshotSource, // Default, actual source stored in file
  };
}
...
export async function getSnapshotContent(
  ...
  const snapshotPath = join(projectRoot, ".snapshots", category, `${filenamePart}.md`);
...
export async function restoreSnapshot(
  projectRoot: string,
  snapshotId: string,
  mcpClient?: { writeIdentityFile: (cat: SnapshotCategory, file: string, content: string, root: string) => Promise<boolean> }
  ...
  try {
    ...
    await createSnapshot(projectRoot, category, filename, currentContent, "pre-replace", "psycheros");
  } catch {
  ...
  try {
    if (mcpClient) {
      await mcpClient.writeIdentityFile(category, filename, content, projectRoot);
    } else {
      await Deno.mkdir(join(projectRoot, "identity", category), { recursive: true });
